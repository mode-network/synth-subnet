...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('validator_scores_prompts_id', sa.BigInteger, nullable=False),
    ...
    op.create_index('ix_validator_scores_prompts_scored_time', 'validator_scores_prompts', ['scored_time'])


def downgrade() -> None:
    op.drop_index('ix_validator_scores_prompts_scored_time', table_name='validator_scores_prompts')
    op.drop_index('ix_miner_scores_miner_uid', table_name='miner_scores')
    op.drop_constraint('fk_miner_scores_miner_predictions_id', 'miner_scores', type_='foreignkey')
    op.drop_constraint('fk_miner_scores_validator_scores_prompts_id', 'miner_scores', type_='foreignkey')
