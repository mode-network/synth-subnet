...
async def buy_dtao(subtensor, wallet, amount_per_block):
    ...
    try:
        ...
        # Execute purchase
        success = await subtensor.buy_dtao(
            wallet=wallet,
        ...
        if success:
            ...
            logger.info(
                f"Successfully purchased {amount_per_block} rao of dTAO "
                f"(Cost: {cost} rao)"
            )
...
async def main():
    ...
    amount_per_block = rao(amount_per_block_rao)
    ...
    while True:
        try:
            await buy_dtao(subtensor, wallet, amount_per_block)
        except Exception as e:
